...
const Skills = () => {
  ...
  return (
    <section id="skills" className='min-h-[42rem] py-[5rem] px-3 text-gray-dd'>
      ...
      <h3 className='text-sm text-center text-l-secondary dark:text-d-secondary'>My Technical Level</h3>

      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2 justify-items-center mt-[4rem]">
        ...
        {skillCategories.map(category => (
          <div className="border-[0.025rem] text-l-secondary dark:text-d-secondary w-full sm:w-[25rem] rounded-3xl">
            <div key={category} className="px-4 py-4 sm:px-8 sm:py-5 ">
              <h4 className="text-l-primary dark:text-d-primary text-lg font-medium p-2 text-center capitalize">{category} Developer</h4>

              <div className="grid grid-cols-2 gap-5 mt-5">
                {user?.skills?.map(skill => (
                  <>
                    {skill.category.toLowerCase() === category.toLowerCase() && (
                      <div key={skill.skillName} className="flex gap-1 text-l-primary dark:text-d-primary">
